...
// ---- Athlete ----
export const getMyAssignments = ({ start, end, filter, date } = {}) =>
  client.get('/my-assignments', { params: { start, end, filter, date } });
...
export const getGames = ({ start, end } = {}) => client.get('/games', { params: { start, end } });

// ---- Coach ----
export const createExercise = ({ name, description, targetDurationMinutes }) =>
  client.post('/exercises', { name, description, targetDuration: targetDurationMinutes });
...
export const addTeamAthlete = (email) => client.post('/team/add-member', { email });
...
export const assignExercise = ({ exerciseId, athleteId, dueDate }) =>
  client.post('/assignments', { exerciseId, memberId: athleteId, dueDate });
...
export const updateAssignmentDueDate = (id, dueDate) =>
  client.put(`/assignments/${id}`, { dueDate });
